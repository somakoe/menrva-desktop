  ...
  Pages.prototype.initHorizontalMenu = function () {
    ...
    function createWrapperLI(hMenu) {
      var li = hMenu
        .children('ul')
        .append(
          "<li class='more'><a href='javascript:;'><span class='title d-flex'><i class='pg-icon'>more_horizontal</i></span></a><ul></ul></li>"
        );
        ...
        $(this)
          .select2({
            minimumResultsForSearch:
              $(this).attr('data-disable-search') == 'true' ? -1 : 1,
          })
        ...
        showErrors: function (errorMap, errorList) {
          ...
          return $.each(errorList, function (index, value) {
            ...
            _popover = $(value.element).popover({
              ...
              html: true,
              template:
                '<div class="popover validation-popover" role="tooltip"><div class="arrow"></div><div class="popover-body"></div></div>',
...
(function (window) {
  ...
  SelectFx.prototype._toggleSelect = function () {
    ...
    if (this._isOpen()) {
      ...
      var scaleH =
        contentWidth > originalWidth ? contentWidth / originalWidth : 1.05;
...
(function ($) {
    ...
    if (refresh) {
      ...
      if (this.options.progress == 'circle') {
        ...
        if (!toggle.find('[class$="-animated"]').length) {
          ...
          iconNew.addClass(
            'card-icon-refresh-lg-' + this.options.progressColor + '-animated'
          );
  ...
  $(document).on('click.pg.card.data-api', '[data-toggle="close"]', function (
    e
  ) {
    var $this = $(this);
    var $target = $this.closest('.card');
  ...
  $(document).on('click.pg.card.data-api', '[data-toggle="refresh"]', function (
    e
  ) {
    var $this = $(this);
    var $target = $this.closest('.card');
...
(function ($) {
  ...
  var Quickview = function (element, options) {
    ...
    $(this.options.notes).on('click', '.list > ul > li .form-check', function (
      e
    ) {
      e.stopPropagation();
    });
...
(function ($) {
  ...
  Parallax.prototype.animate = function () {
    ...
    this.$coverPhoto.css({
      transform:
        direction + '(' + scrollPos * this.options.speed.coverPhoto + 'px)',
    });

    this.$content.css({
      transform:
        direction + '(' + scrollPos * this.options.speed.content + 'px)',
    });
  ...
  var Sidebar = function (element, options) {
    ...
    $.Pages.getUserAgent() == 'desktop' &&
      this.$sidebarMenu.scrollbar({
        ignoreOverlay: false,
        disableBodyScroll:
          this.$element.data('disableBodyScroll') == true ? true : false,
      });
